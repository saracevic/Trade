...
class TradingPair(BaseModel):
    ...
    model_config = {"json_encoders": {datetime: lambda v: v.isoformat()}, "use_enum_values": True}
...
class ScannerConfig(BaseModel):
    ...
    enabled_exchanges: List[str] = Field(default_factory=lambda: [e.value for e in ExchangeType])
    ...
    def validate_exchanges(cls, v: List[str]) -> List[str]:
        ...
        for exchange in v:
            if exchange not in valid_exchanges:
                raise ValueError(
                    f"Exchange '{exchange}' not supported. " f"Must be one of {valid_exchanges}"
                )
        return v
